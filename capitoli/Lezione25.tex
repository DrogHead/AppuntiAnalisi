...
\begin{document}
	...
	\begin{proof}
		...
		Dimostriamo il teorema. Sia $\phi$ un funzionale lineare limitato di $C_0(X)$, di norma $1$. Vogliamo costruire un altro funzionale positivo $\Lambda$ sulle gunzioni di $C_c$ tale che $\abs{\phi f}\leq \Lambda\abs{f} \leq \norm{f}$. Per il teorema di Riesz associamo a $\Lambda$ una misura di Borel $\lambda$, che è regolare se $\lambda(X)<+\infty$.
		\begin{equation}
		...
		\begin{equation}
			\phi(f) = \int_X fgd\lambd \implies \abs{\phi(f)} \leq \int_X \abs{f}\abs{g}d\lambda \leq \int_X \abs{g}d\lambda \leq \int_X d\lambda = \lambda(X)
		\end{equation}
		??? $\lambda(X)\geq 1$ ??? e quindi $\lambda(X)=1$ e $\abs{g}=1$. Concludiamo che $d\abs{\mu}=\abs{g}d\lambda = d\lambda$ e come conseguenza di Radon--Nikodym abbiamo $\abs{\mu}(X)=\lambda(X)=1=\norm{\phi}$.
		...
		\end{equation}
		Manca solo da estendere il risultato a tutto $C_c$, che in realtà viene dal fatto che $f=f^+ - f^-$ e di conseguenza $2f^+ =\abs{f}+f$ e $2f^-=\abs{f}-f$ e quindi usare il risultato precedente, da cui $\Lambda f=\Lambdaf^+ - \Lambda f^-$.  
