...
\begin{document}
	% lezione del 18/09
	\begin{oss} Considerando una successione $\{a_n\}_{n\in \N}\subset \R $ si ha  che $lim\ sup a_n = lim_n \ sup_{k\geq n} a_k = inf_n sup_{k\geq n} a_k = inf_n g_n$ fissando la successione decrescente $ g_n \coloneq sup_{k\geq n} a_k$.\\
	...
	\paragraph{Esercizio } Sia $\{f_n(x)\}_{n\in \N}$ una successione di funzioni misurabili, sia $A \subseteq X$  l'insieme di convergenza $\implies \ A \in \mathscr{M}$.
	...
	\begin{theorem}
		Sia $(X,\mathscr{M)$ e $f: X \to [0,\infty)$ funzione misurabile, allora $\exists \{f_n(x)\} $ successione monotona crescente di funzioni semplici positive tali che $f_n(x) \xrightarrow{n\to \infty}f(x)$ .
	\end{theorem}
	
	\begin{proof}
		Dividiamo in step la dimostrazione . 
		
		%%  DIMOSTRAZIONE DA RUBARE AD AN.MA. III
	...
	\begin{proof}
		\begin{enumerate}
			...
			\item Dato che $A_i \subset \bigcup_{i=1}^infty $ , applicando il punto precendete è immediato l'asserto. \\
					Considerando la successione $ C_1= A_1, \  C_2 = A_2/A_1, \dots , C_n= A_n /A_{n-1} $ tale che $C_i \cap C_j = \empty \ forall i\ni j $ e si ha che $ \bicap_i A_i = \bigcap_i C_i$. Dunque $\mu(\bigcap_{i=1}^\infty A_i) = \mu (\bigcap_{i=1}^\infty C_i) = \sum_{i=1}^\infty _i  = lim_n \sum_{i=1}^n \mu(C_i)= lim_n \mu (\bigcap_{i=1}^n C_i) = lim_n \mu (\bigcap_{i=1}^n A_i)$.
	...
	\begin{definition} [Integrale di funzione semplice]
		SIa $f(x)= \sum_{i=1}^N \alpha_i \rchi_A_i $ una funzione semplice positiva  (con $\alpha_i \geq 0 \ \forall i \in I_N$) , si definisce l'integrale $\int_X f d\mu =  \sum_{i=1}^N \alpha_i \mu(A_i) \quad \forall A \in \mathscr(M)$.
	\end{definition}
	...
	\begin{prop} [Proprietà dell'integrale]
		...
		\item Sia $f>\geq$ e sia $\infty>c\geq0$ una costante , allora $\int_A c d d\mu = c \int_A d\mu  \quad \forall A \in \mathscr{M]$;
	...
	\begin{proof}
		...
		Quindi: $	\int_X f(x) \ d\mu > \int_E_n f_n(x) \ d\mu \geq \int_E_n c s(x) \ d\mu = c \int_E_n s(x) \  d\mu  	$
		Sia $	\nu(A) \coloneq \int_A s(x) \ d\mu 	$ una misura ( osservazione precedente ), riprendendo il passaggio precedente si ha $	\int_X f(x) \ d\mu >c \int_E_n s(x) \  d\mu	= c \nu(E_n )$ ; ma si ha che, per monotonia dell'integrale, $\nu(X) = \nu (\lim_n E_n )= \lim_n \nu(E_n )= \lim_n \int_E_n s(x) \ d\mu $. 
