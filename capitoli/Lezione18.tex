...
\begin{document}
	...
	\begin{prop}
		Se $X$ è uno spazio metrico completo e $f_n(x)$ è una successione di funzioni continue convergenti puntualmente a una funzione $f(x)$ in $X$, allora l'insieme dove $f$ è continua è generico (il suo complemento è di prima categoria, ossia unione numerabile di mai densi).
	\end{prop}
	\begin{proof}
		...
		\end{equation}
		$\omega$ è monotona crescente in $r$ e quindi esiste il suo limite per $r\to 0$. Chiamiamo tale limite \virg{oscillazione di $f$} e lo indichiamo con $\osc(f)$. Si nota immediataamente che $\osc(f)=0\iff f$ è continua. Definiamo l'insieme
		\begin{equation}
			E_\eps = \{x\in X \,\vert\, \osc(f)<\eps\}
		\end{equation}
		che è aperto per ogni $\eps>0$. Infatti, per ogni $x\in E_\eps$ esiste $r>0$ tale che l'estremo superiore $\omega(f)(r,x)$ è minore di $\eps$. Dunque, se consideriamo $x^*\in B_{\frac{r}2}(x)$ l'estremo superiore è ancora minore di $\eps$, e quindi $x^*\in E_\eps$.
		
		Per concludere, mostriamo che per ogni $\eps>0$ e per ogni $B\subset X$ esiste una palla $B_0\subset B$ tale che $\abs{f_n(x)-f(x)}<\eps$ in $B_0$ per qualche $n\in\N$. Consideriamo $\overline{B_1}\subset B$, che è uno spazio metrico completo (chiuso in uno spazio metrico completo), e definiamo
		\begin{equation}
			W_\ell = \{x\in\overline{B_1} \,\vert\, \sup_{j,k\geq\ell}\abs{f_j(x)-f_k(x)}\leq\eps \}
		\end{equation}
		che è chiuso per ogni $\ell$, e inoltre $\overline{B_1}=\bigcup_{\ell\in\N} W_\ell$. Per il lemma di Baire, esiste $m\in\N$ uno dei $W_m$ ha interno non vuoto, e quindi contiene una palla aperta $B_0$. Per costruzione, su $B_0$ l'estremo superiore definito in precedenza è $\leq\eps$. Nel limite per $k\to\infty$, la disuguaglianza diventa:
		\begin{equation}
		...
		\begin{equation}
			F_n=\Big\{x\in X \,\vert\, \osc(f)(x)\geq \frac1n \Big\}
		\end{equation}
	...
	\end{lemma}
	\begin{proof}
		Sia $A$ aperto e sia $y_0\in\Lambda(A)$. Voglio dimostrare che esiste un $\eps>0$ tale che la palla $B_Y(y_0,\eps)\subseteq\Lambda(A)$. Per suriettività esiste sicuramente $x_0\in A$ tale che $y_0=\Lambda x_0$. Dal momento che $A$ è aperto, contiene una palla $B_X(x_0,\delta)$, e abbiamo, anche usando la linearità,
		\begin{align}
			&\Lambda B_X(0,\delta) \supseteq B_Y(0,r\delta) \\
			\implies& \Lambda (x_0+ B_X(0,\delta)) = y_0+\Lambda B_X(0,\delta) \supseteq y_0+B_Y(0,r\delta) = B_Y(y_0,r\delta)
		\end{align}
	\end{proof}
	...
	\end{theorem}
	\begin{proof}
		Faremo vedere un risultato equivalente, ossia mostriamo che esiste $r>0$ tale che $\Lambda(B_X(0,1))\supseteq B_Y(0,r)$.
	
		Per suriettività, $Y=\Lambda(X)=\bigcup_{n\in\N}\Lambda(B_X(0,n))$. Per il lemma di Baire, esiste un $n\in\N$ tale che $\overline{\Lambda(B_X(0,n))}$ ha interno non vuoto. Riscalando e usando la linearità, possiamo assumere $n=1$. Allora $\exists y_0\in Y$ tale che $B_Y(y_0,\eps)\subseteq\overline{\Lambda(B_X(0,1))}$ per qualche $\eps>0$. Allora, per ogni $y\in Y$ tale che $y\in B_Y(0,\eps)$, si ha che $y_0+y\in\overline{\Lambda(B_X(0,1))}$. Si trova quindi facilmente che $y=y_0+y-y_0\in\overline{\Lambda(B_X(0,2))}$. Per chiusura esiste una successione $\suc{x'}{n}{\N}\subset B(0,1)$ tale che $\Lambda x'_n \to y+y_0$, ed esiste un'altra successione $\suc{x''}{n}{\N}\subset B(0,1)$ tale che $\Lambda x''_n\to y_0$. Definita allora $\overline{x}_n=x''_n-x'_n$, si trova che $\overline{x}_n\in B_X(0,2)$ e $\Lambda\overline{x}_n\to y \in \overline{\Lambda(B_X(0,2))}$. Abbiamo mostrato che
		\begin{equation}
			B_Y(0,\eps) \subseteq \overline{\Lambda(B_X(0,2))} \iff B_Y(0,\frac{\eps}2)\subseteq\overline{\Lambda(B_X(0,1))}
		\end{equation}
		Come volevasi dimostrare. Inoltre, la disuguaglianza resta vera anche se moltiplichiamo per una costante. Quindi, per ogni $k\in\N$ si ha che
		\begin{equation}
			B_Y(0,2^{-k}r)\subseteq\overline{\Lambda(B_X(0,2^{-k}))}
		\end{equation}
		Scegliendo $k=1$ troviamo il raggio $\frac{r}2$, esiste $x_1\in B_X(0,\frac12)$ tale che
		\begin{equation}
			y-\Lambda x_1 \in B_Y(0,\frac{r}4)
		\end{equation}
		...
		\end{equation}
		Ossia per definizione e per continuità $\Lambda \sum_{i=1}^k \to y$, che ci dà
		\begin{equation}
			B_Y(0,\frac{r}2) \subseteq \Lambda B_X(0,1)
		\end{equation}
	\end{proof}
